...
This value is added to the route as a prefix, i.e. `http://localhost:3000/api/v1/list_layers`. Using a prefix allows for easy versioning if a route is modified by incrementing the number. If a `v1` and `v2` exist for the same route, they will both be documented.
